...
3. **Support:** We will evaluate how well you support the user in the "notes" section of the json. If some preferences can not be met it should be written in there. For example even if some classes are not assigned, or some professors have free hours, the user should be notified. If some classes were not scheduled the user should know why. Make sure to not be repetitive in the notes. The suggestions should be more about professors than to classes or sections (for example, do not say to change the class time, but say to change the professor preference)
